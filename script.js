...
function getRandomChoice(exclude) {
    let choices = roles.filter(role => role !== exclude && (isFirstTurn && role !== 'Kiún'));
...
function playTurn(childChoice) {
    ...
    // 勝敗判定
    let resultMessage = '';
    if (childChoice === 'Fre' && ['Ye', 'Ch’e', 'Nge'].includes(parentChoice)) {
